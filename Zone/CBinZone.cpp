...
bool CBinZone::ChunkToData()
{
	// ZONEBOUDNS
	{
		...
		{
			g_LogDummy.ELog("Chunk with ZoneBounds Tag 0x%X not found.", ChunkTag);
			return false;
		}
	...
	// PORTAL
	{
		...
		if (m_mapChunk.find(ChunkTag) == m_mapChunk.end())
		{
			g_LogDummy.ELog("Chunk with Portal Tag 0x%X not found.", ChunkTag);
			return false;
		}
		std::string strChunkData = m_mapChunk[ChunkTag];
		int chunkDataSize = strChunkData.size();
	...
	// SPAWN
	{
		...
		if (m_mapChunk.find(ChunkTag) == m_mapChunk.end())
		{
			g_LogDummy.ELog("Chunk with Spawn Tag 0x%X not found.", ChunkTag);
			return false;
		}
		std::string strChunkData = m_mapChunk[ChunkTag];
		int chunkDataSize = strChunkData.size();
	...
	// TRIGGER
	{
		...
		if (m_mapChunk.find(ChunkTag) == m_mapChunk.end())
		{
			g_LogDummy.ELog("Chunk with Trigger Tag 0x%X not found.", ChunkTag);
			return false;
		}
		std::string strChunkData = m_mapChunk[ChunkTag];
		int chunkDataSize = strChunkData.size();
