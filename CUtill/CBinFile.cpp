...
bool CBinFile::Open(const char* pszFileName)
{
	...
	if (fp == nullptr)
	{
		g_LogDummy.ELog("Not Open File %s", pszFileName);
		return false;
	}
...
bool CBinFile::BinChunkRead()
{
	...
	{
		g_LogDummy.ELog("Invalid Magic Number: 0x%X != 0x%X", binFileHeader.Magic, MAPTOOL_MAGIC);
		return false;
	}
	if (binFileHeader.Version != MAPTOOL_VERSION)
	{
		g_LogDummy.ELog("Unsupported Version: %d != %d", binFileHeader.Version, MAPTOOL_VERSION);
		return false;
	}
